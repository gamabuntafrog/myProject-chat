...
const Navbar: FC = memo(() => {
    ...
    const {screenType, isMobileOrTablet} = useGetTypeOfScreen()
    ...
    return (
        <AppBar sx={appBar(screenType, userStyles.backgroundColor)} >
            <Toolbar sx={{px: 1}}>
                {!isMobileOrTablet &&
                ...
                {chatInfo &&
                    <Box sx={{mx: 'auto', ...chatInfoStyles, cursor: 'pointer'}} onClick={() => handleChatInfoIsOpen(isChatInfoOpen)}>
                        <Avatar sx={isMobileOrTablet ? {width: '40px', height: '40px', mr: 1} : {width: '50px', height: '50px', mr: 2}} src={chatInfo.chatImage} alt="avatar" />
                        <Box sx={{textAlign: 'center'}}>
	                        <Typography sx={{mb: -0.5}} variant={isMobileOrTablet ? 'h6' : 'h5'} ><EllipsisText text={chatInfo.chatName} length={30}/></Typography>
                ...
                <IconButton
                    ...
                    color="inherit"
                    aria-label="menu"
                    sx={{padding: isMobileOrTablet ? 0 : 1, ml: user ? 0 : 'auto'}}
                >

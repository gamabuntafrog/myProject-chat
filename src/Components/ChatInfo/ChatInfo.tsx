...
type ChatInfoPT = {
	chatData: chatType
	users: any,
	showRepliedMessage: (message: messageType, actionType: showRepliedMessageActionTypes) => void,
...
const ChatInfo: FC<ChatInfoPT> = memo(
	({
		...
		const [messagesWithImages, setMessagesWithImages] = useState<null | (messageType)[]>(null);
		...
		const seeAMessage = (message: messageType) => {
				...
				<Modal jc={"start"} br={"10px 0 0 10px"} buttonPosition={"absolute"} isModalOpen={isChatInfoOpen} onClose={() => handleChatInfoIsOpen(isChatInfoOpen)}>
					<Box sx={{ textAlign: "center", width: isChangingChatInfo ? "95%" : "auto" }}>
						{isChangingChatInfo ?
							...
							<>
								<Avatar sx={{
									width: "100%", mb: 3, mx: "auto",
									borderRadius: "0 0 30px 30px", height: "auto"
								}} src={chatImage}/>
						...
						<Box sx={{ my: 2 }}>
							...
							<ImageList sx={isMobile ? { width: "100%", maxHeight: "400px", overflowY: "auto" } : { px: 1, maxHeight: "400px", overflowY: "auto" }} cols={isMobile ? 2 : 4}>
								{messagesWithImages.map((message: messageType, i: number) => {
